...
type BlockchainCheckInDelegate = PrismaClient["blockchainCheckIn"];
...
export class GameService implements OnModuleInit {
  ...
  private getDayDurationMs(settings: GameSettings): number {
    const secondsPerDay =
      settings.secondsPerDay && settings.secondsPerDay > 0
        ? settings.secondsPerDay
        : 86400;
  ...
  private async getGameDayInfo(settings: GameSettings) {
    ...
    if (settings.secondsPerDay && settings.secondsPerDay > 0) {
      ...
      const daysSinceLaunch = Math.max(
        0,
        Math.floor((nowMs - launchMs) / dayMs),
      );
      ...
      const todayStart =
        await this.timezoneService.getStartOfDayInTimezone(now);
      if (!todayStart) {
        throw new Error("Failed to get start of today");
      }

      // Get launch date start of day in WordPress timezone
      const launchStart = await this.timezoneService.getStartOfDayInTimezone(
        settings.launchDate,
      );
      if (!launchStart) {
      ...
      const daysSinceLaunch = Math.max(
        0,
        Math.floor((todayStart.getTime() - launchStart.getTime()) / dayMs),
      );
   ...
   */
  private async normalizeToVirtualDay(
    date: Date,
    ...
    if (settings.secondsPerDay && settings.secondsPerDay > 0) {
      ...
      const daysSinceLaunch = Math.max(
        0,
        Math.floor((date.getTime() - launchMs) / dayMs),
      );
      ...
      const normalized =
        await this.timezoneService.getStartOfDayInTimezone(date);
  ...
  async getPlayerStatus(walletAddress: string): Promise<GetPlayerStatusDto> {
    try {
      ...
      const { today, yesterday, nextDayStart, daysSinceLaunch } =
        await this.getGameDayInfo(settings);
      ...
      const normalizedLaunchDate = await this.normalizeToVirtualDay(
        today,
        settings,
      );
      ...
      const bonusTotals = await this.bonusService.getBonusTotals(
      ...
      const safeTotalExtraPlaysUsed = Math.min(
        totalExtraPlaysUsed,
        totalPlaysUsed,
      );
      ...
      if (referral && referralPlaysUsed > totalPlaysUsed) {
        console.error(
          "[Referral] DATA INCONSISTENCY DETECTED - capping referralPlaysUsed:",
          {
      ...
      const basePlaysUsed = Math.max(
        0,
        totalPlaysUsed - safeTotalExtraPlaysUsed,
      );
      ...
      if (playsRemaining <= 0) {
        ...
        secondsToNextPlay = Math.max(
          0,
          Math.floor((nextDayStart.getTime() - Date.now()) / 1000),
        );
      }
      ...
      const normalizedYesterday = await this.normalizeToVirtualDay(
        yesterday,
        settings,
      );
      ...
      const playedYesterday = await Promise.all(
        recentSessions.map(async (session) => {
          ...
          const normalizedSessionDate = await this.normalizeToVirtualDay(
            sessionDate,
            settings,
          );
          return normalizedSessionDate.getTime() ===
            normalizedYesterday.getTime()
            ? session
            : null;
        }),
      ...
      const hasValidStreak =
        playedYesterday !== null || player.lastPlayDate === null;
      ...
      const displayScore = useWeeklyScores
        ? (player.weeklyScore ?? 0)
        : player.totalScore;
      const storedStreak = useWeeklyScores
        ? (player.weeklyStreak ?? 0)
        : player.currentStreak;
      const displayLongestStreak = useWeeklyScores
        ? (player.weeklyLongestStreak ?? 0)
      ...
      const lastPlay = player.lastPlayDate
        ? new Date(player.lastPlayDate)
        : null;
      const normalizedLastPlay = lastPlay
        ? await this.normalizeToVirtualDay(lastPlay, settings)
        : null;

      // Check if this would be your first play today
      const wouldBeFirstPlayToday =
        !normalizedLastPlay ||
        normalizedLastPlay.getTime() < normalizedToday.getTime();
      ...
      if (wouldBeFirstPlayToday) {
        if (!normalizedLastPlay) {
          ...
          normalizedLastPlay.getTime() === normalizedYesterday.getTime()
        ) {
      ...
      const result: GetPlayerStatusDto = {
        ...
        streakMultiplier: this.calculateStreakMultiplier(
          settings,
          projectedMultiplierStreak,
          wouldBeFirstPlayToday,
        ),
        hasValidStreak,
        nextAvailableAt,
        secondsToNextPlay,
      ...
      if (settings.secondsPerDay && settings.secondsPerDay > 0) {
        ...
        if (player.lastPlayDate) {
          const normalizedLastPlay = await this.normalizeToVirtualDay(
            new Date(player.lastPlayDate),
            settings,
          );
          const launchMs = (await this.getGameDayInfo(settings)).launchMs;
          lastPlayVirtualDay = Math.floor(
            (normalizedLastPlay.getTime() - launchMs) / dayMs,
          );
        }
        ...
        const debugInfo: any = {
          ...
          launchDate: (settings.launchDate instanceof Date
            ? settings.launchDate
            : new Date(settings.launchDate)
          ).toISOString(),
        };

        // Add week number if weekly resets are enabled
        if (settings.weeklyResetEnabled) {
          debugInfo.currentWeekNumber = await this.calculateWeekNumber(
            settings,
            today,
          );
        }
      ...
      console.error(
        `[GameService] Error getting player status for ${walletAddress}:`,
        error,
      );
  ...
  async submitScore(dto: SubmitScoreDto): Promise<GameSession> {
    try {
      ...
      const isFirstPlayToday =
        !normalizedLastPlay || normalizedLastPlay.getTime() < today.getTime();
      ...
      const currentStreak = isWeeklyResetEnabled
        ? (player.weeklyStreak ?? 0)
        : player.currentStreak;
      ...
      if (isFirstPlayToday) {
        ...
        try {
          ...
          if (prismaError?.code === "P2002") {
            ...
            console.log(
              `[GameService] Streak already recorded for player ${player.id} on ${today.toISOString()}`,
            );
          } else {
      ...
      this.updateActivePlayersMetric().catch((err) => {
        console.error(
          "[Metrics] Error updating active players after score submission:",
          err,
        );
      });

      // Calculate week number for this session
      const weekNumber = settings.weeklyResetEnabled
        ? await this.calculateWeekNumber(settings, today)
        : null;
      ...
      const useReferralPlay =
        referral && referral.extraPlaysUsed < referral.extraPlaysTotal;
      ...
      if (useReferralPlay) {
        ...
        const usedBonusPlay = await this.bonusService.markBonusPlayUsed(
          player.walletAddress.toLowerCase(),
        );
        if (usedBonusPlay) {
          console.log(
      ...
      try {
        const store = (
          this.cacheManager as Cache & {
            store?: Store & { mdel?: (...keys: string[]) => Promise<void> };
          }
        ).store;
        if (
          store &&
          ...
          if (leaderboardKeys && leaderboardKeys.length > 0) {
            ...
            console.log(
              `[Cache] Invalidated ${leaderboardKeys.length} leaderboard cache keys`,
            );
          }
        }
      } catch (cacheError) {
        console.log(
          `[Cache] Error invalidating leaderboard cache:`,
          cacheError,
        );
      ...
      // Log unexpected errors for debugging
      console.error(
        `[GameService] Error submitting score for ${dto.walletAddress}:`,
        error,
      );
      ...
      if (prismaError?.code === "P2002") {
        ...
        throw new HttpException(
          "Score submission conflict - please try again",
          HttpStatus.CONFLICT,
        );
  ...
  async getLeaderboard(limit = 10, page = 1, userWalletAddress?: string) {
    ...
    const userCacheKey = userWalletAddress
      ? `leaderboard:${mode}:user:${userWalletAddress}`
      : null;
    ...
    try {
      ...
      const cachedUser = userCacheKey
        ? await this.cacheManager.get<any>(userCacheKey)
        : null;
      ...
      if (cachedPage && (!userWalletAddress || cachedUser)) {
        ...
        this.metricsService.cacheHits.inc({
          cache_key_pattern: "leaderboard:*",
        });
        ...
        console.log(
          `[Cache] ❌ Leaderboard cache MISS: ${pageCacheKey} - fetching from DB`,
        );
    ...
    if (userWalletAddress) {
      ...
      if (user) {
        const userScore = useWeeklyScores
          ? (user.weeklyScore ?? 0)
          : user.totalScore;
        ...
        const userStreak = useWeeklyScores
          ? (user.weeklyStreak ?? 0)
          : user.currentStreak;
        const userLongestStreak = useWeeklyScores
          ? (user.weeklyLongestStreak ?? 0)
          : user.longestStreak;
    ...
    const leaderboard = players.map((p, index) => {
      ...
      const longestStreak = useWeeklyScores
        ? (p.weeklyLongestStreak ?? 0)
        : p.longestStreak;
    ...
    if (useWeeklyScores) {
      nextResetTime = (
        await this.calculateNextResetTime(settings)
      ).toISOString();
    }
    ...
    try {
      ...
      console.log(
        `[Cache] ✅ Cached leaderboard page: ${pageCacheKey} (TTL: 30s)`,
      );
      ...
      if (userCacheKey && userWalletAddress) {
        ...
        console.log(
          `[Cache] ✅ Cached user leaderboard data: ${userCacheKey} (TTL: 30s)`,
        );
      }
    } catch (cacheError) {
    ...
    try {
      ...
      if (
        ...
        if (playerKeys && playerKeys.length > 0) {
          ...
          console.log(
            `[Cache] Invalidated ${playerKeys.length} player cache keys after weekly reset`,
          );
        }
      }
    } catch (err) {
      console.error(
        "[Cache] Error invalidating player caches after weekly reset:",
        err,
      );
  ...
  private calculateStreakMultiplier(
    ...
    if (isForScoreSubmission) {
      ...
      const raw =
        settings.streakBaseMultiplier + streak * settings.streakIncrementPerDay;
      ...
      const raw =
        ...
        (streak - 1) * settings.streakIncrementPerDay;
      return Math.min(raw, 2.0);
    }
  }
   ...
   * Uses WordPress timezone for production mode (same as livestream system).
   */
  private async calculateWeekNumber(
    settings: GameSettings,
    date: Date,
  ): Promise<number> {
    if (settings.secondsPerDay && settings.secondsPerDay > 0) {
      ...
      const launchDateTz = await this.timezoneService.getStartOfDayInTimezone(
        settings.launchDate,
      );
      if (!launchDateTz) {
      ...
      const weeksSinceLaunch = Math.floor(
        (weekStart.getTime() - launchWeekStart.getTime()) / weekMs,
      );
  ...
  async getSettings(): Promise<GameSettings> {
    ...
    try {
      ...
      if (cached) {
        this.metricsService.cacheHits.inc({
          cache_key_pattern: "game:settings",
        });
        return cached;
      }
      this.metricsService.cacheMisses.inc({
    ...
    if (this.settingsFetchInProgress) {
      console.log(
        "[getSettings] In-flight fetch detected — awaiting existing WordPress request",
      );
      return this.settingsFetchInProgress;
    }

    console.log("[getSettings] Cache miss — fetching from WordPress...");
    this.settingsFetchInProgress = this.fetchAndCacheSettings(
      cacheKey,
      fallbackKey,
    ).finally(() => {
   ...
   */
  private async fetchAndCacheSettings(
    cacheKey: string,
    fallbackKey: string,
    ...
    if (!wpSettings || !wpSettings.launchDate) {
      ...
      try {
        ...
        if (stale) {
          console.warn(
            "[getSettings] ⚠️ WordPress unavailable — serving stale settings from fallback cache",
          );
          return stale;
        }
      } catch (fallbackError) {
        console.error(
          "[getSettings] Error reading fallback cache:",
          fallbackError,
        );
      }
      throw new Error(
    ...
    let dbSettings = await this.prisma.gameSettings.findUnique({
      where: { id: 1 },
    });
    if (!dbSettings) {
    ...
    try {
      ...
      console.error(
        `[Cache] ❌ CRITICAL: Error storing in cache:`,
        err.message,
      );
      throw new Error(
        `Cache write failed: ${err.message}. Cannot proceed without Redis cache.`,
      );
    }
  ...
  /**
   * Normalize value from settings (Date or ISO string after Redis cache round-trip) to Date.
   */
  ...
  async getGameState(): Promise<{
    ...
    dailyCheckInLaunchDate: string | null;
  }> {
    try {
      const settings = await this.getSettings();
      // After Redis cache round-trip, dates are ISO strings; normalize for comparison and response
      const launchDate = GameService.toDate(settings.launchDate);
      const dailyLaunch =
        settings.dailyCheckInLaunchDate != null
          ? GameService.toDate(settings.dailyCheckInLaunchDate)
          : null;
      const now = new Date();
      return {
        ...
        dailyCheckInEnabled: Boolean(settings.dailyCheckInEnabled),
        dailyCheckInLaunchDate:
          dailyLaunch != null ? dailyLaunch.toISOString() : null,
      };
    } catch (error: unknown) {
      ...
      console.error(
        "[GameService] getGameState failed — returning degraded response:",
        error,
      );
      return {
        gameState: "ACTIVE",
  ...
  async getCheckInStatus(walletAddress: string): Promise<{
    ...
    streak: number;
    nextCheckInScore: number;
  }> {
    // Normalize address for a stable cache key regardless of EIP-55 casing
    const normalizedAddress = walletAddress.toLowerCase();
    const cacheKey = `checkin:status:${normalizedAddress}`;
    ...
    const cacheGetDuration = (Date.now() - cacheStart) / 1000;

    this.metricsService.cacheOperations.observe(
      { operation: "get", cache_key_pattern: "checkin:status:*" },
      cacheGetDuration,
    );

    if (cached) {
      this.metricsService.cacheHits.inc({ cache_key_pattern: "checkin:status:*" });
      this.metricsService.checkInStatusRequests.inc({
        checked_in_today: String(cached.checkedInToday),
      });
      return cached;
    }
    ...
    const CHECKIN_INCREMENT = 0.2;
    const CHECKIN_MAX_MULTIPLIER = 3.0;
    const nextMultiplier = Math.min(
      1.0 + streak * CHECKIN_INCREMENT,
      CHECKIN_MAX_MULTIPLIER,
    );
    const nextCheckInScore = Math.round(CHECKIN_BASE * nextMultiplier);

    const result = { checkedInToday, checkInDates, nextResetTime, streak, nextCheckInScore };

    // TTL: cap at 60s, but always expire just after the next UTC midnight so the
    // "checked in today" flag is never stale across a day boundary.
    const ttlMs = Math.min(60_000, nextResetMs - nowMs + 2_000);
    const cacheSetStart = Date.now();
    await this.cacheManager.set(cacheKey, result, ttlMs);
    this.metricsService.cacheOperations.observe(
      { operation: "set", cache_key_pattern: "checkin:status:*" },
      (Date.now() - cacheSetStart) / 1000,
    );

    this.metricsService.checkInStatusRequests.inc({
      checked_in_today: String(checkedInToday),
    });
